  ...
  return (
    <div className="et-app-root" style={{fontFamily:"'DM Sans',system-ui,sans-serif",display:"flex",height:"100vh",overflow:"hidden",background:"#F5F7FB"}}>
      ...
      {modulo===0&&(tab===7||tab===8||tab===9)&&(["admin","coordinador","ejecutor"].includes(role))&&(()=>{
        ...
        const odtsRol=role==="ejecutor"?odtsBaseFiltradas.filter(isAssignedOdt):odtsBaseFiltradas;
        const canManageOdt=isAdmin||isCoord||isSolicitante;
        const canNotifyOdt=(o)=>canManageOdt||isAssignedOdt(o);
          ...
          <div className="odt-responsive-root" style={{padding:"clamp(10px,2vw,20px) clamp(10px,2.5vw,24px) 48px",background:"#f0f4f8",minHeight:"100%",overflowX:"hidden"}}>
            ...
            {subT==="nueva"&&(
              <div style={{...SH,maxWidth:1040,padding:"clamp(14px,2.4vw,24px)",margin:"0 auto",overflow:"hidden"}}>
                ...
                {(()=>{const hasBrief=Boolean((odtFormDraft.titulo||odtForm.titulo||"").trim()&&odtForm.fechaInicio&&odtForm.fechaEntrega&&odtForm.horaCorte);const hasAsignacion=Boolean(hasBrief&&odtForm.disenadorId);const hasConfirmar=Boolean(hasAsignacion&&odtForm.tipo);return(
                  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:28}}>
                    {["Brief","Asignación","Confirmar"].map((s,i)=>{
                      const active=i===0||(i===1&&hasBrief)||(i===2&&hasConfirmar);
                      const cur=(i===0&&!hasBrief)||(i===1&&hasBrief&&!hasConfirmar)||(i===2&&hasConfirmar);
                          ...
                          {colItems.map(o=>{
                            ...
                            return(
                              <div key={o.id} style={{background:isHL?"rgba(108,110,245,.07)":"#fff",borderRadius:12,border:"1px solid #e2e8f0",borderLeft:`3px solid ${col.c}`,padding:12,boxShadow:"0 1px 4px rgba(0,0,0,.05)",transition:"background .4s"}}>
                                ...
                                {canManageOdt&&o.estado==="pendiente"&&!o.disenadorId&&<button onClick={()=>setOdtAssignModal(o)} style={{marginTop:8,width:"100%",padding:"5px 0",borderRadius:8,border:"1px solid #6C6EF5",background:"#EEEFFE",color:"#6C6EF5",fontSize:10,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:5}}>
